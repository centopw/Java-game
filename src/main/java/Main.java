...
public class Main extends Canvas implements Runnable{
    ...
    public void run(){
        ...
        requestFocus();      
        while(running){
            long currenTimeInNanoSeconds = System.nanoTime();
            updatesToPerform += (currenTimeInNanoSeconds - lastTimeInNanoSeconds) / nanoSecondsPerUpdate;
            if(updatesToPerform >= 1){
                update();
                updates++;
                updatesToPerform--;
            }
            lastTimeInNanoSeconds = currenTimeInNanoSeconds;
